 ...
 */
class NeoBet extends Controller
{
    /**
     *
     */
    ...
    private function transform($match)
    {
        ...
        $time = date('Y-m-d H:i:s', strtotime($match->begin));
     ...
     * @return string
     */
    private function checkLive($match)
    {
        if ($match->tradingType === 'Live') {
            return '1';
        }
        return '0';
    }
     ...
     * @return mixed
     */
    public function liveSort()
    {
        $allMatches = $this->parse();
        $tempArray = [];
        foreach ($allMatches as $index => $match) {
            if ($match['isLive'] === '1') {
                array_push($tempArray, $match);
            }
        }
        return $finalArray['$stakes'] = $tempArray;
    ...
    public function notLiveSort()
    {
        ...
        $finalArray['$stakes'] = $tempArray;
        return $finalArray['$stakes'];
    }
}
